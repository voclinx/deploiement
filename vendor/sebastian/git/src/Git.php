...
class Git
{
    ...
    {
        $this->execute('clone ' . $url . ' ' . $this->repositoryPath);
    }
    /**
     * "Pull" command
     */
     ...
     * "config" command
     */
    public function config($config, $content)
    {
        $this->execute('config '.$config.' "'.$content.'"');
    }
    /**
     * Add files to commit
     *
    ...
    {
        $command = 'cd ' . escapeshellarg($this->repositoryPath) . ' && D:\Git\bin\git.exe ' . $command . ' 2>&1';
