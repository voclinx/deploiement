...
class controller_opcaim
{
    ...
    public function livraison()
    {
        ...
        if (isset($_POST['environnement']) && isset($_POST['vers']) &&
            ...
            $json_data = $db->getLivraison($_GET['id']);
            if(isset($json_data['commitId'])&&$json_data['commitId']!=null){
                $commitId = $json_data['commitId'];
            }else{
                $commitId = null;
            }
            $env = str_replace(" ","\\n",$_POST['environnement']);
            ...
            exec('php ' . __DIR__ . "/deploiement.script.php " . $env . " " . $version . " " . $projectId . " " . $commitId, $output);
            $this->deployment($_GET['id'],$_POST['environnement']);
//            print_r($output);
        }
    }
 ...
 */
    ...
    public function deployment($idProject, $env){
        $db = new db();
        $db->deployment($idProject,$env);
    }
    ...
    public function getCommitId($idDeploy){
        $db = new db();
        return $db->getCommitId($idDeploy);
    }

    public function getDateDeployment($idDeployment){
        $db = new db();
        return $db->getDateDeployment($idDeployment);
    }

    public function getRapportLivraison($idDeployment){
        $db = new db();
        return $db->getRapportLivraison($idDeployment);
    }
}
?>
