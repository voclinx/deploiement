...
<div class="right_col" role="main">
    ...
    <div class="jumbotron row ">
        ...
        <div class="row">
            ...
            <?php if (isset($data["commitId"]) && $data["commitId"] != null) : ?>
                <div class="col-md-offset-2 col-md-4">
                    <div class="form-group ">
                        <label for="comment">Commentaires de retour du deploiement :</label>
                        <textarea class="form-control" rows="2" id="commentaire_rapport"
                                  name="commentaire_rapport"><?php if ($validation == 1 and isset($data["commentaires_retours_installation"])) print $data["commentaires_retours_installation"] ?></textarea>
                    </div>
                </div>
            <?php endif; ?>
    ...
    <div class="jumbotron row">
        ...
        if (count($branch['hardis']) != 0) {
            ...
            foreach ($branch['hardis'] as $item) {
                ...
                print('<label><input type="radio" name="environnement" VALUE="' . substr(strrchr($item, '/'), 1) . '"');
                if (isset($data['environnement']) && $data['environnement'] == substr(strrchr($item, '/'), 1) && $validation == 1) {
    ...
    <input type="hidden" name="lastVersion"
           value="<?php if ($validation == 1 and isset($data["lastVersion"])) print $data["lastVersion"] ?>">
    <input type="hidden" name="deploy"
           value="<?php if ($validation == 1 and isset($data["deploy"])) print $data["deploy"] ?>">
    <input type="hidden" name="commitId"
           value="<?php if ($validation == 1 and isset($data["commitId"])) print $data["commitId"] ?>">
    <input type="hidden" name="deploymentDate"
           value="<?php if ($validation == 1 and isset($data["deploymentDate"])) print $data["deploymentDate"] ?>">
